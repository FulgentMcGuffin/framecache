...
that :class:`FrameCache` requires.  Two concrete implementations are shipped:

* :class:`RedisBackend` – thin wrapper around a live ``redis.Redis`` client;
  TTL is handled natively by Redis.
* :class:`SQLiteBackend` – stores entries in a local SQLite file (or
  ``":memory:"`` for tests).  TTL is enforced lazily via an ``expires_at``
  column, and expired rows are pruned on every ``scan`` call and on ``close``.
  Uses WAL journal mode for concurrent-read safety.

Both backends implement the same Protocol so :class:`FrameCache` is agnostic to
the underlying storage.  Extra helpers on :class:`SQLiteBackend`
(e.g. :meth:`~SQLiteBackend.metadata_df`) expose richer introspection that Redis
cannot provide.
"""
...
class SQLiteBackend:
    ...
    def _glob_to_sql_like(pattern: str) -> str:
        ...
        ``\\x``→ ``x``       (Redis-glob-escaped literal — strip the backslash,
                              then re-escape if necessary for SQL)
        """
        result: list[str] = []
        i = 0
        while i < len(pattern):
            ...
            if ch == "\\" and i + 1 < len(pattern):
                # Redis-glob-escaped literal character
                nxt = pattern[i + 1]
                if nxt == "%":
                    result.append("\\%")
                elif nxt == "_":
                    ...
                    result.append(nxt)  # *, ?, [, ] → literal (safe in LIKE)
                i += 2
            elif ch == "*":
                result.append("%")
                i += 1
    ...
    def set(self, key: str, value: bytes, ttl: timedelta | None = None) -> None:
        now = self._now_iso()
        expires_at = self._expiry_iso(ttl) if ttl is not None else None
        self._connection.execute(
            """
